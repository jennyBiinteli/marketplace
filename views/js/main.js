    ...
    if (targetServerProducts.length > 0) {
      $(targetServerProducts).DataTable({
        ...
        ajax: {
          url:
            $("#path").val() +
            "ajax/data-products.php?idStore=" +
            $("#idStore").val(),
          type: "POST",
        },
        columns: [
    ...
    if (targetServerDisputes.length > 0) {
      $(targetServerDisputes).DataTable({
        ...
        ajax: {
          url:
            $("#path").val() +
            "ajax/data-disputes.php?idStore=" +
            $("#idStore").val() +
            "&token=" +
            localStorage.getItem("token_user"),
          type: "POST",
    ...
    if (targetServerMessages.length > 0) {
      $(targetServerMessages).DataTable({
        ...
        ajax: {
          url:
            $("#path").val() +
            "ajax/data-messages.php?idStore=" +
            $("#idStore").val() +
            "&token=" +
            localStorage.getItem("token_user"),
          type: "POST",
      ...
      target.each(function (i) {
        ...
        $(el).ready(function () {
          var settings = {
            url:
              $("#urlApi").val() +
              "products?linkTo=id_product&equalTo=" +
              $(el).attr("idProduct") +
              "&select=description_product",
            method: "GET",
            timeout: 0,
          };
          ...
          $.ajax(settings).done(function (response) {
            if (response.status == 200) {
              $(el).summernote(
                "pasteHTML",
                response.results[0].description_product
              );
            }
          });
        });
    ...
    if (response.status == 200) {
      ...
      if (type == "email") {
        $(event.target)
          ...
          .html(
            "The email already exists in the database and was registered with the method " +
              response.results[0].method_user
          );
      }
      ...
      if (type == "store" || type == "product") {
        $(event.target)
          ...
          .html(
            "The name " +
              type +
              " already exists in the database and was registered"
          );
      }
  ...
  if (type == "text") {
    ...
    if (!pattern.test(event.target.value)) {
      ...
      $(event.target)
        .parent()
        .children(".invalid-feedback")
        .html("Do not use numbers or special characters");
  ...
  if (localStorage.getItem("token_user") != null) {
    ...
    var settings = {
      url:
        ...
        "users?equalTo=" +
        token +
        "&linkTo=token_user&select=id_user,wishlist_user",
      method: "GET",
      timeout: 0,
    };
...
function removeWishlist(urlProduct, urlApi, urlDomain) {
  fncSweetAlert("confirm", "Are you sure to delete this item?", "").then(
    (resp) => {
      if (resp) {
        /*=============================================
            Revisar que el token coincida con la base de datos
        ...
        var settings = {
          url:
            urlApi +
            "users?linkTo=token_user&equalTo=" +
            localStorage.getItem("token_user") +
            "&select=id_user,wishlist_user",
          method: "GET",
          timeout: 0,
        };
...
function addShoppingCart(urlProduct, urlApi, currentUrl, tag) {
  ...
  var settings = {
    url:
      urlApi +
      "products?linkTo=url_product&equalTo=" +
      urlProduct +
      "&select=" +
      select,
    method: "GET",
  ...
  $.ajax(settings).done(function (response) {
    if (response.status == 200) {
      ...
      if (response.results[0].stock_product == 0) {
        fncSweetAlert("error", "Out of Stock", "");
      ...
      if (details == "") {
        if (response.results[0].specifications_product != null) {
          var detailsProduct = JSON.parse(
            response.results[0].specifications_product
          );
          ...
          for (const i in detailsProduct) {
            ...
            details +=
              '"' + property + '":"' + detailsProduct[i][property][0] + '",';
          }
      ...
      if (count != listCookies.length) {
        if (arrayListSC != undefined) {
          ...
          for (const i in arrayListSC) {
            if (
              arrayListSC[i].product == urlProduct &&
        ...
        fncSweetAlert("success", "Product added to Shopping Cart", currentUrl);
      }
    }
  });
}
...
function totalP(index) {
  ...
  if (price.length > 0) {
    price.each(function (i) {
      ...
      if (index != null) {
        $(shipping[index]).html(
          Number($(shipping[index]).attr("currentShipping")) *
            Number($(quantity[index]).val())
        );
      } else {
        /*=============================================
        ...
        $(shipping[i]).html(
          Number($(shipping[i]).attr("currentShipping")) *
      ...
      let sub =
        Number($(price[i]).html()) * Number($(quantity[i]).val()) +
        Number($(shipping[i]).html());
      total += sub;
...
function newOrder(methodPaid, statusPaid, idPayment, total) {
  ...
  var phoneOrder =
    $("#countryOrder").val().split("_")[1] + "_" + $("#phoneOrder").val();
          ...
          if (response.status == 200) {
            ...
            if (forEachEnd == idProduct.length) {
              ...
              if (methodPaid == "payu") {
                ...
                // var apiKey = "4j3yGlxVWDKw2S3ZZ3zuCjUWsA";
...
function changeCategory(event) {
  ...
  var settings = {
    url:
      $("#urlApi").val() +
      "subcategories?equalTo=" +
      idCategory +
      "&linkTo=id_category_subcategory&select=id_subcategory,name_subcategory,title_list_subcategory",
    method: "GET",
    timeout: 0,
  };
  ...
  $.ajax(settings).done(function (response) {
    ...
    response.results.forEach((item) => {
      $('[name="subCategoryProduct"]').append(
        `<option class="optSubCategory" value="` +
          item.id_subcategory +
          `_` +
          item.title_list_subcategory +
          `">` +
                ...
                <div class="form-group__content input-group mb-3 inputSummary">
                    ...
                    <div class="input-group-append">
                        <span class="input-group-text">
                             <button type="button" class="btn btn-danger"  onclick="removeInput(` +
          inputs.length +
          `,'inputSummary')">&times;</button>
                        </span>
                    </div>
                    ...
                    <div class="col-12 col-lg-6 form-group__content input-group">
                        ...
                        <div class="input-group-append">
                            <span class="input-group-text">
                                 <button type="button" class="btn btn-danger" onclick="removeInput(` +
          inputs.length +
          `,'inputSpecifications')">&times;</button>
                            </span>
                        </div>
                        ...
                        class="form-control" 
                        type="text"
                        name="specificationsTypeProduct_` +
          inputs.length +
          `"
                        pattern='[-\\(\\)\\=\\%\\&\\$\\;\\_\\*\\"\\#\\?\\¿\\!\\¡\\:\\,\\.\\0-9a-zA-ZñÑáéíóúÁÉÍÓÚ ]{1,}'
                        onchange="validateJS(event,'paragraphs')"
                    ...
                    <div class="col-12 col-lg-6 form-group__content input-group">
                        ...
                        data-role="tagsinput"
                        type="text"
                        name="specificationsValuesProduct_` +
          inputs.length +
          `"
...
if (
  $("[name='galleryProductOld']").length > 0 &&
  $("[name='galleryProductOld']").val() != ""
) {
    ...
    if (value.status == "ok") {
      ...
      status =
        `<div class="col-10 mt-3">
                ...
                <input type="hidden" name="idStore" value="` +
        idStore +
        `">
        ...
        $(this).attr("processOrder") +
        `">
                <input type="hidden" name="idOrder" value="` +
        idOrder +
        `">
                <input type="hidden" name="clientOrder" value="` +
        clientOrder +
        `">
                <input type="hidden" name="emailOrder" value="` +
        emailOrder +
        `">
...
/*=============================================
Función para quickview del producto
=============================================*/
function quickview(item) {
  // console.log(item);
  $("#modal-name").text(item.name_product);
  $("#modal-name-store").text(item.name_store);
  $("#modal-price").text(item.price_product);  
  document.getElementById("modal-url").href = getPath("")+ item.url_product; 
  document.getElementById("show-more").href = getPath("")+ item.url_product; 
  
 

  let ruta = "img/products/" + item.url_category + "/" + item.image_product;
  document.getElementById("modal-image").src = ruta;

  if (item.stock_product == 0) {
    let html2 = document.getElementById("ps-sold-out").innerHTML;
    html2 =`status:<strong class="ps-tag--out-stock"> Out of stock</strong>`;
    const modal = document.querySelector("#ps-sold-out");

    modal.innerHTML = html2;
  }

  if (item.summary_product != "") {
    let summary = JSON.parse(item.summary_product);
    let html = "";
    summary.forEach((element) => {
      console.log(element);
      html = html + `<li> ` + element + ` </li>`;
    });
    const data = document.querySelector("#modal-summary");
    data.innerHTML = html;
  }

  
  // let html = `<a class="btn"
  ...
  // <i class="icon-bag2"></i>
  // </a>`;
  // const btnAdd = document.getElementById("ps-product__shopping");
  // btnAdd.innerHTML = html;

  // if(item.offer_product != ""){
  //   let discount = JSON.parse(item.offer_product);
  ...
  // $("#product-quickview").setAttribute(name, value);
}
function getPath(url){
  let PORT = location.port;
  url = url +  "http://marketplace.com:"+PORT+"/";
  return url
}
